    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          ...
          <p className="text-muted-foreground text-lg">Browse and play unblocked games seamlessly.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-24 bg-card/30 rounded-3xl border border-white/5 border-dashed">
          ...
          <h3 className="text-xl font-display font-semibold mb-2">No games found</h3>
          <p className="text-muted-foreground">Try adjusting your search or add games in the Admin panel.</p>
        </div>
      ) : (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {games.map((game, i) => (
            <motion.button
              ...
              onClick={() => openUrl(game.url, game.name)}
            ...
            >
              ...
              <div className="w-20 h-20 mb-4 rounded-2xl bg-white/5 flex items-center justify-center text-4xl shadow-inner group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
                {game.icon ? (
                  ...
                  <span className="font-display font-bold text-primary">{game.name.charAt(0)}</span>
                )}
              </div>
              
              <h3 className="font-display font-bold text-lg leading-tight mb-1 truncate w-full">{game.name}</h3>
